...
async function runIntegrationTest() {
  ...
  try {
    ...
    let fetchCount = 0;
    const trackFetch = (p: Promise<any>) =>
      p.then((res) => {
        fetchCount++;
    ...
    const [roadsGeoJSON, waterGeoJSON, parksGeoJSON] = await Promise.all([
      ...
      trackFetch(
        fetchFeatures([coords.latitude, coords.longitude], radius, { natural: "water" }, "water")
      ),
      trackFetch(
        fetchFeatures([coords.latitude, coords.longitude], radius, { leisure: "park" }, "parks")
      ),
    ]);
    ...
    const updateTaskProgress = () => {
      ...
      progress.report(
        currentPercent,
        `Worker 已处理完成任务: ${processedShardsCount}/${totalShards}`
      );
    };

    const roadProcessingPromises = roadShards.map((shard, i) =>
      runInWorker(workers[i % numWorkers], "roads", shard).then((res) => {
        updateTaskProgress();
        return res;
      })
    );
    const waterPromise = runInWorker(workers[0], "polygons", waterTyped).then((res) => {
      updateTaskProgress();
      return res;
    });
    const parksPromise = runInWorker(workers[1], "polygons", parksTyped).then((res) => {
      updateTaskProgress();
      return res;
    });
