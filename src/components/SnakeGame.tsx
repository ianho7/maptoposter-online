...
export default function SnakeGame({
    ...
    const t: SnakeTheme = typeof theme === 'string'
        ? (THEMES[theme] ?? THEMES.paper)
        : theme
    ...
    const tick = useCallback(() => {
        ...
        if (head.x === foodRef.current.x && head.y === foodRef.current.y) {
            const n = scoreRef.current + 1; scoreRef.current = n; setScore(n)
    ...
    const close = useCallback(() => {
        ...
        timerRef.current && clearInterval(timerRef.current)
        phaseRef.current = 'idle'; setPhase('idle'); setScore(0); handleSetOpen(false)
    }, [handleSetOpen])
    ...
    useEffect(() => {
        if (!open) return
        snakeRef.current = mkSnake(); foodRef.current = spawnFood(snakeRef.current)
        scoreRef.current = 0; setScore(0)
...
function DPadBtn({ col, row, label, color, onPress }: {
    col: number; row: number; label: string; color: string; onPress: () => void
}) {
    return (
        <button
    ...
    return (
        <svg width="10" height="10" viewBox="0 0 10 10" fill={color} opacity={0.6}>
            {[0, 4, 8].flatMap(x => [0, 4, 8].map(y => (
                <rect key={`${x}-${y}`} x={x} y={y} width={2} height={2} rx={0.5} />
            )))}
        </svg>
    )
}